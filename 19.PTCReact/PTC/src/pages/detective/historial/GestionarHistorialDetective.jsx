...
const GestionarHistorialDetective = ({ historial, onEditar }) => {
    ...
    <Paper elevation={1} sx={{ p: 2, mb: 2 }}>
      ...
      {editMode ? (
        ...
        <Typography variant="body1">
          {editableHistorial[field] || "No disponible"}
        </Typography>
      )}
    </Paper>
  );
  ...
  const renderEditableNestedField = (label, section, field, icon) => (
    <Paper elevation={1} sx={{ p: 2, mb: 2 }}>
      ...
      {editMode ? (
        ...
        <Typography variant="body1">
          {editableHistorial[section]?.[field] || "No disponible"}
        </Typography>
      )}
    </Paper>
  );
  ...
  return (
    <Box sx={{ px: { xs: 2, sm: 4 }, py: 3 }}>
      ...
      <Accordion>
        ...
        <AccordionDetails>
          {editableHistorial.idCaso?.evidencias?.length > 0 ? (
            <Stack spacing={2}>
              {editableHistorial.idCaso.evidencias.map((evidencia, index) => (
...
export default GestionarHistorialDetective;
